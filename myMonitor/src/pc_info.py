...
class pc_info ( wx.Frame ):
	...
	def __init__( self, parent ):
		wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = wx.EmptyString, pos = wx.DefaultPosition, size = wx.Size( 1207,838 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
		...
		self.m_manufa = wx.StaticText( self, wx.ID_ANY, u"MyLabel", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_ostype = wx.StaticText( self, wx.ID_ANY, u"MyLabel", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_cpunum = wx.StaticText( self, wx.ID_ANY, u"MyLabel", wx.DefaultPosition, wx.DefaultSize, 0 )
	...
	def getSysInfo(self):
		if(sys.platform == "win32"):
			...
			self.m_manufa.SetLabel(CS.Manufacturer)
			self.m_ostype.SetLabel(CS.Model)
			self.m_cpunum.SetLabel(str(psutil.cpu_count()))
			self.m_cpu.SetLabel(CPU.Name)
			self.m_memory.SetLabel(str(int(CS.TotalPhysicalMemory) // 1024**3) + u"GB")
			self.m_freememory.SetLabel(str(int(OS.FreePhysicalMemory) // 1024) + u"MB")
			# self.m_memory.SetLabel(str(int(OS.TotalVisibleMemorySize)/1024/1024)+"MB")
